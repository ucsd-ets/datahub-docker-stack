...
def test_gpu_valid():
    assert run(cer_path=path_to_cer,timeout=60) == EXAMPLE_GOOD_OUT
    
def test_gpu_nonexistent_image():
    assert run(test_image='invalid_image_for_test',cer_path=path_to_cer,timeout=20) == EXAMPLE_TIME_OUT
    
def test_gpu_lacking_tools():
    assert run(test_image='python',cer_path=path_to_cer,timeout=60) == EXAMPLE_MISSING_PACKAGE

def test_env_cert_valid():
    with open(path_to_cer, 'rb') as f:
        os.environ['GRPC_CERT']=f.read().decode()
    assert run(cer_path=None,timeout=60) == EXAMPLE_GOOD_OUT

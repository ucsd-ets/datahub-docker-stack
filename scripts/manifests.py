...
def compile_history(compile_tag_history = False):
    ...
    """
    # glob(path.join()) will return a list of manifests md in random order, causing mismatch with images
    ...
    manifests_doc_names = ['ucsdets-' + path.basename(image).replace(':', '-') for image in read_var(img_source)]
